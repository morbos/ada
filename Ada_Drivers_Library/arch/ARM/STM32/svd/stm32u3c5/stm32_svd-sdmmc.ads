...
package STM32_SVD.SDMMC is
   ...
   type SDMMC_STAR_Register is record
      ...
      DABORT         : Boolean;
      --  Read-only. Data path state machine active (not in Idle state)
      DPSMACT        : Boolean;
      --  Read-only. Command path state machine active (not in Idle state)
      CPSMACT        : Boolean;
   ...
   --  SDMMC1 address block description
