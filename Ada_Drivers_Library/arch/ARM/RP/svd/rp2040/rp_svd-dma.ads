...
package RP_SVD.DMA is
   ...
   type CH0_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH0_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      ...
      --  RING_SEL.
      RING_SIZE      : CH0_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
   ...
   type CH1_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH1_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH1_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH2_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH2_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH2_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH3_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH3_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH3_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH4_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH4_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH4_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH5_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH5_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH5_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH6_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH6_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH6_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH7_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH7_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH7_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH8_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH8_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH8_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH9_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH9_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH9_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH10_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH10_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH10_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type CH11_CTRL_TRIG_Register is record
      ...
      --  Set the size of each bus transfer (byte/halfword/word). READ_ADDR and
      --  WRITE_ADDR advance by this amount (1/2/4 bytes) with each transfer.
      DATA_SIZE      : CH11_CTRL_TRIG_DATA_SIZE_Field := RP_SVD.DMA.Size_Byte;
      --  If 1, the read address increments with each transfer. If 0, each read
      --  is directed to the same, initial address.\n\n Generally this should
      --  be disabled for peripheral-to-memory transfers.
      INCR_READ      : Boolean := False;
      ...
      --  This can apply to either read or write addresses, based on value of
      --  RING_SEL.
      RING_SIZE      : CH11_CTRL_TRIG_RING_SIZE_Field := RP_SVD.DMA.Ring_None;
      --  Select whether RING_SIZE applies to read or write addresses.\n If 0,
      --  read addresses are wrapped on a (1 << RING_SIZE) boundary. If 1,
      --  write addresses are wrapped.
      RING_SEL       : Boolean := False;
   ...
   type SNIFF_CTRL_Register is record
      ...
      CALC           : SNIFF_CTRL_CALC_Field := RP_SVD.DMA.Crc32;
