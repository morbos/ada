...
package STM32_SVD.USART is
   ...
   subtype USART_CR1_enabled_DEDT_Field is HAL.UInt5;
   subtype USART_CR1_enabled_DEAT_Field is HAL.UInt5;

   --  USART control register 1 [alternate]
   type USART_CR1_enabled_Register is record
      ...
      --  be disabled before resetting the UE bit. In Smartcard mode, (SCEN =
      --  1), the CK is always available when CLKEN = 1, regardless of the UE
      --  bit value.
      UE      : Boolean := False;
      ...
      --  before entering low-power mode and clear it when exit from low-power
      --  mode. If the USART does not support the wakeup from Stop feature,
      --  this bit is reserved and must be kept at reset value. Refer to page
      --  1985.
      UESM    : Boolean := False;
      ...
      --  USART_ISR register. In Smartcard mode, when TE is set, there is a 1
      ...
      --  This bitfield can only be written when the USART is disabled (UE =
      --  0).
      PS      : Boolean := False;
      ...
      --  computed parity is inserted at the MSB position (9th bit if M = 1;
      --  8th bit if M = 0) and the parity is checked on the received data.
      --  This bit is set and cleared by software. Once it is set, PCE is
      --  active after the current byte (in reception and in transmission).
      --  This bitfield can only be written when the USART is disabled (UE =
      --  0).
      PCE     : Boolean := False;
      --  Receiver wakeup method This bit determines the USART wakeup method
      ...
      --  Refer to .
      DEDT    : USART_CR1_enabled_DEDT_Field := 16#0#;
      ...
      DEAT    : USART_CR1_enabled_DEAT_Field := 16#0#;
      ...
      --  Note: FIFO mode can be used on standard UART communication, in SPI
   ...
   for USART_CR1_enabled_Register use record
      UE      at 0 range 0 .. 0;
      UESM    at 0 range 1 .. 1;
      RE      at 0 range 2 .. 2;
      TE      at 0 range 3 .. 3;
   ...
   subtype USART_CR1_disabled_DEDT_Field is HAL.UInt5;
   subtype USART_CR1_disabled_DEAT_Field is HAL.UInt5;

   --  USART control register 1 [alternate]
   type USART_CR1_disabled_Register is record
      --  USART enable When this bit is cleared, the USART prescalers and
      --  outputs are stopped immediately, and all current operations are
      --  discarded. The USART configuration is kept, but all the USART_ISR
      --  status flags are reset. This bit is set and cleared by software.
      ...
      --  The DMA requests are also reset when UE = 0 so the DMA channel must
      --  be disabled before resetting the UE bit. In Smartcard mode, (SCEN =
      --  1), the CK pin is always available when CLKEN = 1, regardless of the
      --  UE bit value.
      UE             : Boolean := False;
      --  USART enable in low-power mode When this bit is cleared, the USART
      --  cannot wake up the MCU from low-power mode. When this bit is set, the
      ...
      --  supported, this bit is reserved and must be kept at reset value.
      --  Refer to .
      DEDT           : USART_CR1_disabled_DEDT_Field := 16#0#;
      --  Driver Enable assertion time This 5-bit value defines the time
      --  between the activation of the DE (Driver Enable) signal and the
      --  beginning of the start bit. It is expressed in sample time units (1/8
      --  or 1/16 bit time, depending on the oversampling rate). This bitfield
      ...
      --  Driver Enable feature is not supported, this bit is reserved and must
      --  be kept at reset value. Refer to .
      DEAT           : USART_CR1_disabled_DEAT_Field := 16#0#;
      --  Receiver timeout interrupt enable This bit is set and cleared by
      --  software. Note: If the USART does not support the Receiver timeout
      --  feature, this bit is reserved and must be kept at reset value. .
      RTOIE          : Boolean := False;
   ...
   for USART_CR1_disabled_Register use record
      UE             at 0 range 0 .. 0;
      UESM           at 0 range 1 .. 1;
      RE             at 0 range 2 .. 2;
      TE             at 0 range 3 .. 3;
   ...
   type USART_CR2_Register is record
      ...
      --  Note: In 7-bit and 9-bit data modes, the address detection is done on
      --  6-bit and 8-bit address (ADD[5:0] and ADD[7:0]) respectively.
      ADDM7        : Boolean := False;
      ...
      --  This bit can only be written when the USART is disabled (UE = 0).
      --  Note: If synchronous mode is not supported, this bit is reserved and
      ...
      --  reserved and must be kept at reset value. Refer to . In Smartcard
      --  mode, in order to provide correctly the CK clock to the smartcard,
      --  the steps below must be respected: UE = 0 SCEN = 1 GTPR configuration
      --  CLKEN= 1 UE = 1
      CLKEN        : Boolean := False;
      ...
      --  (UE = 0). Note: If DATAINV = 1 and/or MSBFIRST = 1 the patterns must
      --  be the same on the line, for example 0xAA for MSBFIRST) If the USART
      ...
      --  communication to wakeup from Mute mode with 4-bit/7-bit address mark
      ...
      --  active on address match), the wakeup from low-power mode is performed
      --  when the received character corresponds to the character programmed
      --  through ADD[6:0] or ADD[3:0] bitfield (depending on ADDM7 bit), and
      --  WUF interrupt is enabled by setting WUFIE bit. The MSB of the
      --  character sent by transmitter should be equal to 1. In Run mode with
      --  Mute mode inactive (for example, end-of-block detection in ModBus
      --  protocol): the whole received character (8 bits) is compared to
      --  ADD[7:0] value and CMF flag is set on match. An interrupt is
      --  generated if the CMIE bit is set. These bits can only be written when
      --  the reception is disabled (RE = 0) or when the USART is disabled (UE
      --  = 0).
   ...
   subtype USART_CR3_WUS_Field is HAL.UInt2;
   ...
   type USART_CR3_Register is record
      ...
      --  noise flag or SPI slave underrun error (FE = 1 or ORE = 1 or NE = 1
      --  or UDR = 1 in the USART_ISR register).
      EIE            : Boolean := False;
      --  IrDA mode enable This bit is set and cleared by software. This bit
      --  can only be written when the USART is disabled (UE = 0). Note: If
      --  IrDA mode is not supported, this bit is reserved and must be kept at
      ...
      --  This bitfield can only be written when the USART is disabled (UE =
      --  0). Note: If the USART does not support Smartcard mode, this bit is
      --  reserved and must be kept at reset value. Refer to .
      SCEN           : Boolean := False;
      ...
      --  (UE = 0). Note: If the hardware flow control feature is not
      --  supported, this bit is reserved and must be kept at reset value.
      --  Refer to .
      RTSE           : Boolean := False;
      ...
      --  mode is enabled, the RXFIFO is bypassed and data is written directly
      ...
      --  written when the USART is disabled (UE = 0). Note: If the Driver
      --  Enable feature is not supported, this bit is reserved and must be
      --  kept at reset value. .
      DEM            : Boolean := False;
      ...
      --  disabled (UE = 0). When the USART is enabled (UE = 1), this bitfield
      --  may only be written to 0x0, in order to stop retransmission. Note: If
      --  Smartcard mode is not supported, this bit is reserved and must be
      --  kept at reset value. Refer to .
      SCARCNT        : USART_CR3_SCARCNT_Field := 16#0#;
      --  Wakeup from low-power mode interrupt flag selection This bitfield
      --  specifies the event which activates the WUF (Wakeup from low-power
      --  mode flag). This bitfield can only be written when the USART is
      --  disabled (UE = 0). If the USART does not support the wakeup from Stop
      --  feature, this bit is reserved and must be kept at reset value. Refer
      --  to page 1985.
      WUS            : USART_CR3_WUS_Field := 16#0#;
      --  Wakeup from low-power mode interrupt enable This bit is set and
      --  cleared by software. Note: WUFIE must be set before entering in
      --  low-power mode. If the USART does not support the wakeup from Stop
      --  feature, this bit is reserved and must be kept at reset value. Refer
      --  to page 1985.
      WUFIE          : Boolean := False;
   ...
   type USART_BRR_Register is record
      --  USART baud rate BRR[15:4] BRR[15:4] = USARTDIV[15:4] BRR[3:0] When
      --  OVER8 = 0, BRR[3:0] = USARTDIV[3:0]. When OVER8 = 1: BRR[2:0] =
      --  USARTDIV[3:0] shifted 1 bit to the right. BRR[3] must be kept
      --  cleared.
   ...
   type USART_GTPR_Register is record
      ...
      --  IrDA Normal and Low-Power baud rate PSC[7:0] is used to program the
      --  prescaler for dividing the USART source clock to achieve the
      --  low-power frequency: the source clock is divided by the value given
      --  in the register (8 significant bits): In Smartcard mode: PSC[4:0] =
      --  Prescaler value PSC[4:0] is used to program the prescaler for
      --  dividing the USART source clock to provide the Smartcard clock. The
      --  value given in the register (5 significant bits) is multiplied by 2
      --  to give the division factor of the source clock frequency: ... ...
      --  This bitfield can only be written when the USART is disabled (UE =
      --  0). Note: Bits [7:5] must be kept cleared if Smartcard mode is used.
      --  This bitfield is reserved and forced by hardware to 0 when the
      --  Smartcard and IrDA modes are not supported. Refer to .
   ...
   type USART_RTOR_Register is record
      ...
      --  in terms of number of bits during which there is no activity on the
      --  RX line. In standard mode, the RTOF flag is set if, after the last
      --  received character, no new start bit is detected for more than the
      --  RTO value. In Smartcard mode, this value is used to implement the CWT
      --  and BWT. See Smartcard chapter for more details. In the standard, the
      --  CWT/BWT measurement is done starting from the start bit of the last
      --  received character. Note: This value must only be programmed once per
      --  received character.
      ...
      --  0: 0 information characters + LEC BLEN = 1: 0 information characters
      --  + CRC BLEN = 255: 254 information characters + CRC (total 256
      --  characters)) In Smartcard mode, the Block length counter is reset
      --  when TXE = 0 (TXFE = 0 in case FIFO mode is enabled). This bitfield
      --  can be used also in other modes. In this case, the Block length
      --  counter is reset when RE = 0 (receiver disabled) and/or when the
   ...
   type USART_RQR_Register is record
      ...
      --  ones not yet transmitted, the software should wait for the TXE flag
   ...
   type USART_ISR_enabled_Register is record
      --  Read-only. Parity error This bit is set by hardware when a parity
      --  error occurs in receiver mode. It is cleared by software, writing 1
      ...
      --  EIE = 1 in the USART_CR1 register. Note: This error is associated
      --  with the character in the USART_RDR.
      FE             : Boolean;
      ...
      --  writing 1 to the NECF bit in the USART_ICR register. Note: This bit
      ...
      --  if the EIE bit is set. When the line is noise-free, the NE flag can
      --  be disabled by programming the ONEBIT bit to 1 to increase the USART
      --  tolerance to deviations (Refer to Tolerance of the USART receiver to
      --  clock deviation on page 2012). This error is associated with the
      ...
      --  interrupt is generated if RXFNEIE = 1 or EIE = 1 in the USART_CR1
      --  register. Note: When this bit is set, the USART_RDR register content
      ...
      --  the EIE bit is set. This bit is permanently forced to 0 (no overrun
      --  detection) when the bit OVRDIS is set in the USART_CR3 register.
      ORE            : Boolean;
      ...
      --  until the RXFNE bit has been set (i.e. a new idle line occurs). If
      --  Mute mode is enabled (MME = 1), IDLE is set if the USART is not mute
      --  (RWU = 0), whatever the Mute mode selected by the WAKE bit. If RWU =
      --  1, IDLE is not set.
      IDLE           : Boolean;
      ...
      --  register. It is set by hardware when the transmission of a frame
      --  containing data is complete and when TXFE is set. An interrupt is
      --  generated if TCIE = 1 in the USART_CR1 register. TC bit is is cleared
      --  by software, by writing 1 to the TCCF in the USART_ICR register or by
      --  a write to the USART_TDR register. Note: If TE bit is reset and no
      --  transmission is on going, the TC bit is immediately set.
      TC             : Boolean;
      ...
      --  setting TXFRQ bit), the flag TXFNF should be checked prior to writing
      --  in TXFIFO (TXFNF and TXFE are set at the same time). This bit is used
      --  during single buffer transmission.
      TXFNF          : Boolean;
      ...
      --  nCTS input toggles, if the CTSE bit is set. It is cleared by
      --  software, by writing 1 to the CTSCF bit in the USART_ICR register. An
      --  interrupt is generated if CTSIE = 1 in the USART_CR3 register. Note:
      ...
      --  inverted copy of the status of the nCTS input pin. Note: If the
      ...
      --  depending on the oversampling method), RTOF flag is set. The counter
      --  counts even if RE = 0 but RTOF is set only when RE = 1. If the
      --  timeout has already elapsed when RE is set, then RTOF is set. If the
      --  USART does not support the Receiver timeout feature, this bit is
      --  reserved and kept at reset value.
      RTOF           : Boolean;
      ...
      --  BLEN + 4. An interrupt is generated if the EOBIE = 1 in the USART_CR1
      --  register. It is cleared by software, writing 1 to the EOBCF in the
      ...
      SBKF           : Boolean;
      --  Read-only. Receiver wakeup from Mute mode This bit indicates if the
      --  USART is in Mute mode. It is cleared/set by hardware when a
      --  wakeup/mute sequence is recognized. The Mute mode control sequence
      --  (address or IDLE) is selected by the WAKE bit in the USART_CR1
      --  register. When wakeup on IDLE mode is selected, this bit can only be
      ...
      --  this bit is reserved and kept at reset value. Refer to .
      RWU            : Boolean;
      --  Read-only. Wakeup from low-power mode flag This bit is set by
      --  hardware, when a wakeup event is detected. The event is defined by
      ...
      --  cleared. If the USART does not support the wakeup from Stop feature,
      --  this bit is reserved and kept at reset value. Refer to .
      WUF            : Boolean;
      ...
      --  writing TE = 0, followed by TE = 1 in the USART_CR1 register, in
      --  order to respect the TE = 0 minimum period.
      TEACK          : Boolean;
      ...
      --  the wakeup from Stop feature, this bit is reserved and kept at reset
      --  value. Refer to .
      REACK          : Boolean;
      ...
      --  received data corresponds to RXFIFO size + 1 (RXFIFO full + 1 data in
   ...
   for USART_ISR_enabled_Register use record
      PE             at 0 range 0 .. 0;
      FE             at 0 range 1 .. 1;
      NE             at 0 range 2 .. 2;
      ORE            at 0 range 3 .. 3;
   ...
   --  USART interrupt and status register [alternate]
   type USART_ISR_disabled_Register is record
      --  Read-only. Parity error This bit is set by hardware when a parity
      --  error occurs in receiver mode. It is cleared by software, writing 1
      --  to the PECF in the USART_ICR register. An interrupt is generated if
      ...
      --  when the maximum number of transmit attempts is reached without
      --  success (the card NACKs the data frame). An interrupt is generated if
      --  EIE = 1 in the USART_CR1 register.
      FE             : Boolean;
      --  Read-only. Noise detection flag This bit is set by hardware when
      --  noise is detected on a received frame. It is cleared by software,
      --  writing 1 to the NECF bit in the USART_ICR register. Note: This bit
      ...
      --  be disabled by programming the ONEBIT bit to 1 to increase the USART
      --  tolerance to deviations (Refer to Tolerance of the USART receiver to
      --  clock deviation on page 2012).
      NE             : Boolean;
      --  Read-only. Overrun error This bit is set by hardware when the data
      --  currently being received in the shift register is ready to be
      --  transferred into the USART_RDR register while RXNE = 1. It is cleared
      --  by a software, writing 1 to the ORECF, in the USART_ICR register. An
      --  interrupt is generated if RXNEIE = 1 or EIE = 1 in the USART_CR1
      --  register. Note: When this bit is set, the USART_RDR register content
      --  is not lost but the shift register is overwritten. An interrupt is
      --  generated if the ORE flag is set during multi buffer communication if
      --  the EIE bit is set. This bit is permanently forced to 0 (no overrun
      ...
      --  USART_CR1 register. It is cleared by software, writing 1 to the
      --  IDLECF in the USART_ICR register. Note: The IDLE bit is not set again
      --  until the RXNE bit has been set (i.e. a new idle line occurs). If
      --  Mute mode is enabled (MME = 1), IDLE is set if the USART is not mute
      --  (RWU = 0), whatever the Mute mode selected by the WAKE bit. If RWU =
      --  1, IDLE is not set.
      IDLE           : Boolean;
      ...
      --  by software, by writing 1 to the TCCF in the USART_ICR register or by
      --  a write to the USART_TDR register. Note: If TE bit is reset and no
      --  transmission is on going, the TC bit is set immediately.
      TC             : Boolean;
      ...
      --  USART_RQR register, in order to discard the data (only in Smartcard T
      --  = 0 mode, in case of transmission failure). An interrupt is generated
      ...
      --  completed without success (ABRE = 1) (ABRE, RXNE and FE are also set
      --  in this case) It is cleared by software, in order to request a new
      --  auto baud rate detection, by writing 1 to the ABRRQ in the USART_RQR
      --  register. Note: If the USART does not support the auto baud rate
      --  feature, this bit is reserved and kept at reset value.
   ...
   for USART_ISR_disabled_Register use record
      PE             at 0 range 0 .. 0;
      FE             at 0 range 1 .. 1;
      NE             at 0 range 2 .. 2;
      ORE            at 0 range 3 .. 3;
   ...
   type USART_ICR_Register is record
      ...
      --  support the Receiver timeout feature, this bit is reserved and must
      --  be kept at reset value. Refer to page 1985.
      RTOCF          : Boolean := False;
      ...
      --  Write-only. Wakeup from low-power mode clear flag Writing 1 to this
      --  bit clears the WUF flag in the USART_ISR register. Note: If the USART
      --  does not support the wakeup from Stop feature, this bit is reserved
      --  and must be kept at reset value. Refer to page 1985.
      WUCF           : Boolean := False;
      --  unspecified
   ...
   type USART_RDR_Register is record
      ...
      --  shift register and the internal bus (see ). When receiving with the
      --  parity enabled, the value read in the MSB bit is the received parity
      --  bit.
   ...
   type USART_TDR_Register is record
      ...
      --  internal bus and the output shift register (see ). When transmitting
      --  with the parity enabled (PCE bit set to 1 in the USART_CR1 register),
      --  the value written in the MSB (bit 7 or bit 8 depending on the data
      --  length) has no effect because it is replaced by the parity. Note:
   ...
   type USART_PRESC_Register is record
      ...
      --  prescaler value is 1011 i.e. input clock divided by 256.
   ...
   type USART_Peripheral
     (Discriminent : USART1_Disc := ENABLED)
   is record
      --  USART control register 2
      ...
      case Discriminent is
         when ENABLED =>
            --  USART control register 1 [alternate]
            USART_CR1_enabled : aliased USART_CR1_enabled_Register;
            --  USART interrupt and status register [alternate]
            USART_ISR_enabled : aliased USART_ISR_enabled_Register;
         when DISABLED =>
            --  USART control register 1 [alternate]
            USART_CR1_disabled : aliased USART_CR1_disabled_Register;
            --  USART interrupt and status register [alternate]
            USART_ISR_disabled : aliased USART_ISR_disabled_Register;
      end case;
   end record
   ...
   for USART_Peripheral use record
      ...
      USART_CR1_enabled  at 16#0# range 0 .. 31;
      USART_ISR_enabled  at 16#1C# range 0 .. 31;
      USART_CR1_disabled at 16#0# range 0 .. 31;
      USART_ISR_disabled at 16#1C# range 0 .. 31;
   end record;
