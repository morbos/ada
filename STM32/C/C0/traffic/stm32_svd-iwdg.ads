...
package STM32_SVD.IWDG is
   ...
   type IWDG_KR_Register is record
      ...
      --  IWDG_RLR and IWDG_WINR registers (see ) Writing the key value 0xCCCC
      --  starts the watchdog (except if the hardware watchdog option is
      --  selected)
   ...
   type IWDG_PR_Register is record
      --  Prescaler divider These bits are write access protected see . They
      --  are written by software to select the prescaler divider feeding the
      --  counter clock. PVU bit of the must be reset in order to be able to
      --  change the prescaler divider. Note: Reading this register returns the
      --  prescaler value from the VDD voltage domain. This value may not be up
      --  to date/valid if a write operation to this register is ongoing. For
      --  this reason the value read from this register is valid only when the
      --  PVU bit in the status register (IWDG_SR) is reset.
   ...
   type IWDG_RLR_Register is record
      ...
      --  see . They are written by software to define the value to be loaded
      --  in the watchdog counter each time the value 0xAAAA is written in the
      ...
      --  the datasheet for the timeout information. The RVU bit in the must be
      --  reset to be able to change the reload value. Note: Reading this
      ...
      --  register is ongoing on it. For this reason the value read from this
      --  register is valid only when the RVU bit in the status register
      --  (IWDG_SR) is reset.
   ...
   type IWDG_WINR_Register is record
      ...
      --  see , they contain the high limit of the window value to be compared
      --  with the downcounter. To prevent a reset, the downcounter must be
      --  reloaded when its value is lower than the window register value and
      --  greater than 0x0 The WVU bit in the must be reset in order to be able
      --  to change the reload value. Note: Reading this register returns the
      --  reload value from the VDD voltage domain. This value may not be valid
      --  if a write operation to this register is ongoing. For this reason the
      --  value read from this register is valid only when the WVU bit in the
      --  (IWDG_SR) is reset.
