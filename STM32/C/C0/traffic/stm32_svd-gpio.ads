...
package STM32_SVD.GPIO is
   ...
   type GPIOA_OTYPER_Register is record
      --  Port x configuration for I/O y (y = 15 to 0) These bits are written
      --  by software to configure the I/O output type.
   ...
   type GPIOA_IDR_Register is record
      --  Read-only. Port x input data I/O y (y = 15 to 0) These bits are
      --  read-only. They contain the input value of the corresponding I/O
      --  port.
   ...
   type GPIOA_ODR_Register is record
      --  Port output data I/O y (y = 15 to 0) These bits can be read and
      --  written by software. Note: For atomic bit set/reset, the OD bits can
      --  be individually set and/or reset by writing to the GPIOx_BSRR
      --  register (x = A, B, C, D, F).
   ...
   type GPIOA_BSRR_Register is record
      --  Write-only. Port x set I/O y (y = 15 to 0) These bits are write-only.
      --  A read operation always returns 0x0000.
      BS : GPIOA_BSRR_BS_Field := (As_Array => False, Val => 16#0#);
      --  Write-only. Port x reset I/O y (y = 15 to 0) These bits are
      --  write-only. A read operation always returns 0x0000. Note: If both BSx
      --  and BRx are set, BSx has priority.
   ...
   type GPIOA_LCKR_Register is record
      --  Port x lock I/O pin y (y = 15 to 0) These bits are read/write but can
      --  only be written when the LCKK bit is 0.
      ...
      --  any bit of the port, any read access on the LCKK bit returns 1 until
      --  the next MCU reset or peripheral reset.
      LCKK           : Boolean := False;
      --  unspecified
   ...
   type GPIOA_BRR_Register is record
      --  Write-only. Port x reset I/O y (y = 15 to 0) These bits are
      --  write-only. A read operation always returns 0x0000.
   ...
   subtype GPIOB_MODER_MODE_Element is HAL.UInt2;
   ...
   subtype GPIOB_OSPEEDR_OSPEED_Element is HAL.UInt2;
   ...
   subtype GPIOB_PUPDR_PUPD_Element is HAL.UInt2;
   ...
   subtype GPIOB_AFRL_AFSEL_Element is HAL.UInt4;
   ...
   subtype GPIOB_AFRH_AFSEL_Element is HAL.UInt4;
