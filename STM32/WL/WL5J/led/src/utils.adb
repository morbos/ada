...
is
   ...
   procedure To_Integer_16_From_UInt32 (Result : out Integer_16; Value : UInt32)
   is
      X : Union4;
   begin
      X.UVal := Value;
      Result := X.IArr (1);
   end To_Integer_16_From_UInt32;
   ...
   procedure To_UInt32_From_Integer_16 (Result : out UInt32; Value : Integer_16)
   is
      X : Union4;
   begin
      X.UVal := 0;
   ...
   procedure Convert_Int_To_Hex (Value : Integer; Result : out String)
   is
      --      C : Character;
      C : UInt8;
      Nibble : UInt8;
      ...
      Remainder : UInt32;
      for Remainder'Address use LValue'Address;
      Index : Integer;
   ...
   begin
      ...
      while Remainder /= 0 loop
         Nibble := UInt8 (Remainder mod 16);
         if Nibble < 10 then
            C := Nibble + Character'Pos ('0');
         else
            C := (Nibble - 10) + Character'Pos ('A');
         end if;
         Result (9 - Index) := Character'Val (C);
         Index := Index + 1;
         Remainder := UInt32 (Shift_Right (Remainder, 4));
      end loop;
   end Convert_Int_To_Hex;
