...
--      TAMP_Periph.BKP1R := UInt32 (Closed (Largest_Axis));
      To_UInt32_From_Integer_32 (Result => TAMP_Periph.BKP1R,
                                 Value => Integer_32 (Closed (Largest_Axis)));
      TAMP_Periph.BKP2R := Largest_Axis'Enum_Rep;
      TAMP_Periph.BKP3R := Stance'Enum_Rep;
      TAMP_Periph.BKP4R := 0; -- Counter

      App_Start (Stance);

   else
      TAMP_Periph.BKP4R := TAMP_Periph.BKP4R + 1;
      My_Delay (1_000); --  1 second
      Mag.Read_MAG (Got); --  Toss
      Mag.Read_MAG (Got);
      Axis := Sensor_Data_Range'Enum_Val (TAMP_Periph.BKP2R);
--      Value := Integer (TAMP_Periph.BKP1R);
      To_Integer_32_From_UInt32 (Result => Integer_32 (Value), Value => TAMP_Periph.BKP1R);
      Stance := Door_Stance'Enum_Val (TAMP_Periph.BKP3R);
      if abs (Value - Integer (Got (Axis))) > Epsilon then --  Stance change
         case Stance is
            when Open_Stance =>
               Stance := Closed_Stance;
            when Closed_Stance =>
...
--         App_Start2 (TAMP_Periph.BKP1R, Got (Axis));
         To_UInt32_From_Integer_32 (Result => TAMP_Periph.BKP1R, Value => Integer_32 (Got (Axis)));
--         TAMP_Periph.BKP1R := UInt32 (Got (Axis));
         TAMP_Periph.BKP3R := Stance'Enum_Rep;
         App_Start (Stance);
      else
         if (Stance = Open_Stance) and (TAMP_Periph.BKP4R >= (N_Seconds / Quanta)) then
            App_Start (Stance); --  Persistent nag if door left open
            TAMP_Periph.BKP4R := 0;
         end if;
      end if;
