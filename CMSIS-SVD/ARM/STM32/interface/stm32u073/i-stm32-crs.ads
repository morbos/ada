...
package Interfaces.STM32.CRS is
   ...
   subtype CRS_CR_TRIM_Field is Interfaces.Bit_Types.UInt7;
   ...
   type CRS_CR_Register is record
      ...
      --  Refer to Section15.4.4 for more details.
      ...
      --  HSI48 oscillator smooth trimming The default value of the HSI48
      --  oscillator smooth trimming is 64, which corresponds to the middle of
   ...
   subtype CRS_CFGR_RELOAD_Field is Interfaces.Bit_Types.UInt16;
   subtype CRS_CFGR_FELIM_Field is Interfaces.Bit_Types.Byte;
   subtype CRS_CFGR_SYNCDIV_Field is Interfaces.Bit_Types.UInt3;
   subtype CRS_CFGR_SYNCSRC_Field is Interfaces.Bit_Types.UInt2;

   --  CRS configuration register
   type CRS_CFGR_Register is record
      --  Counter reload value RELOAD is the value to be loaded in the
      --  frequency error counter with each SYNC event. Refer to Section15.4.3
      ...
      --  the CRS_ISR register. Refer to Section15.4.4 for more details about
      --  FECAP evaluation.
      ...
      --  software to select the SYNC signal source (see Table122): Note: When
      --  using USB LPM (Link Power Management) and the device is in Sleep
      --  mode, the periodic USB SOF is not generated by the host. No SYNC
      --  signal is therefore provided to the CRS to calibrate the HSI48
      --  oscillator on the run. To guarantee the required clock precision
      --  after waking up from Sleep mode, the LSE or reference clock on the
      --  GPIOs must be used as SYNC signal.
   ...
   subtype CRS_ISR_FECAP_Field is Interfaces.Bit_Types.UInt16;
   ...
   type CRS_ISR_Register is record
      ...
      --  has to be taken. An interrupt is generated if the ERRIE bit is set in
      ...
      --  frequency error counter reaches value FELIM * 128 and no SYNC is
      --  detected, meaning either that a SYNC pulse was missed, or the
      --  frequency error is too big (internal frequency too high) to be
      --  compensated by adjusting the TRIM value, hence some other action must
      --  be taken. At this point, the frequency error counter is stopped
      --  (waiting for a next SYNC), and an interrupt is generated if the ERRIE
      --  bit is set in the CRS_CR register. It is cleared by software by
      ...
      --  Section15.4.4 for more details about FECAP usage.
