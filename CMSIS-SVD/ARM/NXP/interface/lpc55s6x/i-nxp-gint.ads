...
package Interfaces.NXP.GINT is
   pragma Preelaborate;
   ...
   type CTRL_Register is record
      --  Group interrupt status. This bit is cleared by writing a one to it.
      --  Writing zero has no effect.
      INT           : CTRL_INT_Field := Interfaces.NXP.GINT.No_Request;
      --  Combine enabled inputs for group interrupt
      COMB          : CTRL_COMB_Field := Interfaces.NXP.GINT.Or_k;
      --  Group interrupt trigger
      TRIG          : CTRL_TRIG_Field := Interfaces.NXP.GINT.Edge_Triggered;
      --  unspecified
   ...
   --  GPIO grouped interrupt port 0 polarity register
   type PORT_POL_Registers is array (0 .. 1) of Interfaces.Bit_Types.Word
     with Volatile;

   --  GPIO grouped interrupt port 0 enable register

   --  GPIO grouped interrupt port 0 enable register
   type PORT_ENA_Registers is array (0 .. 1) of Interfaces.Bit_Types.Word
     with Volatile;
