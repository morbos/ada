...
package NXP_SVD.MRT is
   ...
   type INTVAL_LOAD_Field is
     (--  No force load. The load from the INTVALn register to the TIMERn register is
--  processed at the end of the time interval if the repeat mode is selected.
      No_Force_Load,
      --  Force load. The INTVALn interval value IVALUE -1 is immediately loaded into
--  the TIMERn register while TIMERn is running.
      Force_Load)
     with Size => 1;
   ...
   type STAT_INTFLAG_Field is
     (--  No pending interrupt. Writing a zero is equivalent to no operation.
      ...
      --  Pending interrupt. The interrupt is pending because TIMERn has reached the
--  end of the time interval. If the INTEN bit in the CONTROLn is also set to
--  1, the interrupt for timer channel n and the global interrupt are raised.
--  Writing a 1 to this bit clears the interrupt request.
   ...
   type MODCFG_MULTITASK_Field is
     (--  Hardware status mode. In this mode, the INUSE(n) flags for all channels are
--  reset.
   ...
   type IRQ_FLAG_GFLAG0_Field is
     (--  No pending interrupt. Writing a zero is equivalent to no operation.
      No_Pending_Interrupt,
      --  Pending interrupt. The interrupt is pending because TIMER0 has reached the
--  end of the time interval. If the INTEN bit in the CONTROL0 register is also
--  set to 1, the interrupt for timer channel 0 and the global interrupt are
--  raised. Writing a 1 to this bit clears the interrupt request.
      Pending_Interrupt)
     with Size => 1;
