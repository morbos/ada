...
package NXP_SVD.ADC is
   ...
   type STAT_FOF0_Field is
     (--  No result FIFO 0 overflow has occurred since the last time the flag was
--  cleared.
      Fof0_0,
      --  At least one result FIFO 0 overflow has occurred since the last time the
--  flag was cleared.
      Fof0_1)
     with Size => 1;
   ...
   type STAT_FOF1_Field is
     (--  No result FIFO1 overflow has occurred since the last time the flag was
--  cleared.
      Fof1_0,
      --  At least one result FIFO1 overflow has occurred since the last time the
--  flag was cleared.
      Fof1_1)
     with Size => 1;
...
--  associated FIFO.
   ...
   type STAT_ADC_ACTIVE_Field is
     (--  The ADC is IDLE. There are no pending triggers to service and no active
--  commands are being processed.
      Adc_Active_0,
      --  The ADC is processing a conversion, running through the power up delay, or
--  servicing a trigger.
      Adc_Active_1)
     with Size => 1;
   ...
   type STAT_TRGACT_Field is
     (--  Command (sequence) associated with Trigger 0 currently being executed.
      ...
      Trgact_2,
      --  Command (sequence) from the associated Trigger number is currently being
--  executed.
      Trgact_3,
      --  Command (sequence) from the associated Trigger number is currently being
--  executed.
...
--  flag was cleared.
   ...
   type CFG_TPRICTRL_Field is
     (--  If a higher priority trigger is detected during command processing, the
--  current conversion is aborted and the new command specified by the trigger
--  is started.
      Tprictrl_0,
      --  If a higher priority trigger is received during command processing, the
--  current command is stopped after after completing the current conversion.
...
--  serviced.
      Tprictrl_1,
      --  If a higher priority trigger is received during command processing, the
--  current command will be completed (averaging, looping, compare) before
--  servicing the higher priority trigger.
   ...
   type CFG_TRES_Field is
     (--  Trigger sequences interrupted by a high priority trigger exception will not
--  be automatically resumed or restarted.
      Tres_0,
      --  Trigger sequences interrupted by a high priority trigger exception will be
...
--  automatically restarted.
      Tcmdres_0,
      --  Trigger sequences interrupted by a high priority trigger exception will be
--  resumed from the command executing before the exception.
      Tcmdres_1)
     with Size => 1;
...
--  without startup delays (at the cost of higher DC current consumption). A
--  single power up delay (CFG[PUDLY]) is executed immediately once PWREN is
--  set, and any detected trigger does not begin ADC operation until the power
--  up delay time has passed. After this initial delay expires the analog will
--  remain pre-enabled, and no additional delays will be executed.
      Pwren_1)
     with Size => 1;
...
--  interrupts.
      Tcomp_Flag_9,
      --  Every trigger sequence has been completed and every trigger has enabled
--  completion interrupts.
   ...
   type GCC_RDY_Field is
     (--  The gain calibration value is invalid. Run the auto-calibration routine for
--  this value to be written.
      Rdy_0,
...
--  independently.
   ...
   type CMDL1_MODE_Field is
     (--  Standard resolution. Single-ended 12-bit conversion; Differential 13-bit
...
--  conversion with 2's complement output.
   ...
   type CMDH1_CMPEN_Field is
     (--  Compare disabled.
      ...
      Cmpen_2,
      --  Compare enabled. Repeat channel acquisition (sample/convert/compare) until
--  true.
      Cmpen_3)
     with Size => 2;
...
--  command. If lower priority trigger pending, begin command associated with
...
--  lower priority trigger.
   ...
   type RESFIFO_CMDSRC_Field is
     (--  Not a valid value CMDSRC value for a dataword in RESFIFO. 0x0 is only found
--  in initial FIFO state prior to an ADC conversion result dataword being
--  stored to a RESFIFO buffer.
      Cmdsrc_0,
