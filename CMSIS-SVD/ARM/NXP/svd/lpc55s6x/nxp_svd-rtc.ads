...
--  to configuring or initiating any operation of the RTC.
      Not_In_Reset,
      --  In reset. The RTC is held in reset. All register bits within the RTC will
--  be forced to their reset value except the OFD bit. This bit must be cleared
--  before writing to any register in the RTC - including writes to set any of
--  the other bits within this register. Do not attempt to write to any bits of
--  this register at the same time that the reset bit is being cleared.
   ...
   type CTRL_ALARM1HZ_Field is
     (--  No match. No match has occurred on the 1 Hz RTC timer. Writing a 0 has no
--  effect.
      No_Match,
      --  Match. A match condition has occurred on the 1 Hz RTC timer. This flag
--  generates an RTC alarm interrupt request RTC_ALARM which can also wake up
...
--  power-down mode.
   ...
   type CTRL_RTC_EN_Field is
     (--  Disable. The RTC 1 Hz and 1 kHz clocks are shut down and the RTC operation
--  is disabled. This bit should be 0 when writing to load a value in the RTC
--  counter register.
      Disable,
      --  Enable. The 1 Hz RTC clock is running and RTC operation is enabled. This
--  bit must be set to initiate operation of the RTC. The first clock to the
...
--  crystal connected between the RTC_XTALIN and RTC_XTALOUT pins.
      Used,
      --  The RTC Oscillator is in bypass mode. In this mode a clock can be directly
--  input into the RTC_XTALIN pin.
      Bypass)
     with Size => 1;
   ...
   type CTRL_RTC_SUBSEC_ENA_Field is
     (--  The sub-second counter (if implemented) is disabled. This bit is cleared by
--  a system-level POR or BOD reset as well as a by the RTC_ENA bit (bit 7 in
--  this register). On modules not equipped with a sub-second counter, this bit
--  will always read-back as a '0'.
      Power_Up,
      --  The 32 KHz sub-second counter is enabled (if implemented). Counting
--  commences on the start of the first one-second interval after this bit is
--  set. Note: This bit can only be set after the RTC_ENA bit (bit 7) is set by
--  a previous write operation. Note: The RTC sub-second counter must be
--  re-enabled whenever the chip exits deep power-down mode.
      Powered_Down)
     with Size => 1;
