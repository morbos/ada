...
package NXP_SVD.SYSCON is
   ...
   type SDIOCLKCTRL_PHASE_ACTIVE_Field is
     (--  Bypassed.
      Bypassed,
      --  Activates phase shift logic. When active, the clock divider is active and
--  phase delays are enabled.
      Ph_Shift)
     with Size => 1;
   ...
   type COMP_INT_CTRL_INT_SOURCE_Field is
     (--  Select Analog Comparator filtered output as input for interrupt detection.
      Filter_Int,
      --  Select Analog Comparator raw output (unfiltered) as input for interrupt
--  detection. Must be used when Analog comparator is used as wake up source in
--  Power down mode.
      Raw_Int)
     with Size => 1;
   ...
   type SYSCON_Peripheral
     (Discriminent : SYSCON_Disc := Val_0)
   is record
      --  Memory Remap control register
