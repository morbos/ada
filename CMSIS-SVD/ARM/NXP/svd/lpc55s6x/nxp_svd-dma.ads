...
package NXP_SVD.DMA is
   ...
   type CTRL_ENABLE_Field is
     (--  Disabled. The DMA controller is disabled. This clears any triggers that
--  were asserted at the point when disabled, but does not prevent
...
--  triggered, based on TRIGTYPE.
   ...
   type CFG_TRIGTYPE_Field is
     (--  Edge. Hardware trigger is edge triggered. Transfers will be initiated and
--  completed, as specified for a single trigger.
      Edge,
...
--  be used on that channel. Transfers continue as long as the trigger level is
--  asserted. Once the trigger is de-asserted, the transfer will be paused
--  until the trigger is, again, asserted. However, the transfer will not be
--  paused until any remaining transfers within the current BURSTPOWER length
--  are completed.
      Level)
     with Size => 1;
   ...
   type CFG_TRIGBURST_Field is
     (--  Single transfer. Hardware trigger causes a single transfer.
      Single,
      --  Burst transfer. When the trigger for this channel is set to edge triggered,
--  a hardware trigger causes a burst transfer, as defined by BURSTPOWER. When
--  the trigger for this channel is set to level triggered, a hardware trigger
...
--  transfer is complete.
   ...
   type CTLSTAT_TRIG_Field is
     (--  Not triggered. The trigger for this DMA channel is not set. DMA operations
...
--  carried out.
   ...
   type XFERCFG_CFGVALID_Field is
     (--  Not valid. The channel descriptor is not considered valid until validated
--  by an associated SETVALID0 setting.
      Not_Valid,
   ...
   type XFERCFG_RELOAD_Field is
     (--  Disabled. Do not reload the channels' control structure when the current
--  descriptor is exhausted.
      Disabled,
      --  Enabled. Reload the channels' control structure when the current descriptor
--  is exhausted.
      Enabled)
     with Size => 1;
   ...
   type XFERCFG_SWTRIG_Field is
     (--  Not set. When written by software, the trigger for this channel is not set.
--  A new trigger, as defined by the HWTRIGEN, TRIGPOL, and TRIGTYPE will be
--  needed to start the channel.
      Not_Set,
      --  Set. When written by software, the trigger for this channel is set
--  immediately. This feature should not be used with level triggering when
--  TRIGBURST = 0.
      Set)
     with Size => 1;
   ...
   type XFERCFG_CLRTRIG_Field is
     (--  Not cleared. The trigger is not cleared when this descriptor is exhausted.
--  If there is a reload, the next descriptor will be started.
      Not_Cleared,
   ...
   type XFERCFG_SETINTA_Field is
     (--  No effect.
      No_Effect,
      --  Set. The INTA flag for this channel will be set when the current descriptor
--  is exhausted.
      Set)
     with Size => 1;
   ...
   type XFERCFG_SETINTB_Field is
     (--  No effect.
      No_Effect,
      --  Set. The INTB flag for this channel will be set when the current descriptor
--  is exhausted.
      Set)
     with Size => 1;
   ...
   type XFERCFG_WIDTH_Field is
     (--  8-bit. 8-bit transfers are performed (8-bit source reads and destination
--  writes).
      Bit_8,
      --  16-bit. 6-bit transfers are performed (16-bit source reads and destination
--  writes).
      Bit_16,
      --  32-bit. 32-bit transfers are performed (32-bit source reads and destination
--  writes).
      Bit_32)
     with Size => 2;
   ...
   type XFERCFG_SRCINC_Field is
     (--  No increment. The source address is not incremented for each transfer. This
--  is the usual case when the source is a peripheral device.
      No_Increment,
      --  1 x width. The source address is incremented by the amount specified by
--  Width for each transfer. This is the usual case when the source is memory.
      Width_X_1,
...
--  specified by Width for each transfer.
   ...
   type XFERCFG_DSTINC_Field is
     (--  No increment. The destination address is not incremented for each transfer.
--  This is the usual case when the destination is a peripheral device.
      No_Increment,
      --  1 x width. The destination address is incremented by the amount specified
--  by Width for each transfer. This is the usual case when the destination is
--  memory.
      Width_X_1,
      --  2 x width. The destination address is incremented by 2 times the amount
--  specified by Width for each transfer.
      Width_X_2,
      --  4 x width. The destination address is incremented by 4 times the amount
--  specified by Width for each transfer.
      Width_X_4)
     with Size => 2;
