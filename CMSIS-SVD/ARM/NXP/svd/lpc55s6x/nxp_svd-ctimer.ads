...
package NXP_SVD.CTIMER is
   ...
   type TCR_CRST_Field is
     (--  Disabled. Do nothing.
      Disabled,
      --  Enabled. The Timer Counter and the Prescale Counter are synchronously reset
--  on the next positive edge of the APB bus clock. The counters remain reset
--  until TCR[1] is returned to zero.
      Enabled)
     with Size => 1;
   ...
   type EMR_EMC0_Field is
     (--  Do Nothing.
      Do_Nothing,
      --  Clear. Clear the corresponding External Match bit/output to 0 (MAT0 pin is
--  LOW if pinned out).
      Clear,
      --  Set. Set the corresponding External Match bit/output to 1 (MAT0 pin is HIGH
--  if pinned out).
      Set,
   ...
   type EMR_EMC1_Field is
     (--  Do Nothing.
      Do_Nothing,
      --  Clear. Clear the corresponding External Match bit/output to 0 (MAT1 pin is
--  LOW if pinned out).
      Clear,
      --  Set. Set the corresponding External Match bit/output to 1 (MAT1 pin is HIGH
--  if pinned out).
      Set,
      --  Toggle. Toggle the corresponding External Match bit/output.
      Toggle)
   ...
   type EMR_EMC2_Field is
     (--  Do Nothing.
      Do_Nothing,
      --  Clear. Clear the corresponding External Match bit/output to 0 (MAT2 pin is
--  LOW if pinned out).
      Clear,
      --  Set. Set the corresponding External Match bit/output to 1 (MAT2 pin is HIGH
--  if pinned out).
      Set,
      --  Toggle. Toggle the corresponding External Match bit/output.
      Toggle)
   ...
   type EMR_EMC3_Field is
     (--  Do Nothing.
      Do_Nothing,
      --  Clear. Clear the corresponding External Match bit/output to 0 (MAT3 pin is
--  LOW if pinned out).
      Clear,
      --  Set. Set the corresponding External Match bit/output to 1 (MAT3 pin is HIGH
--  if pinned out).
      Set,
      --  Toggle. Toggle the corresponding External Match bit/output.
      Toggle)
   ...
   type CTCR_CTMODE_Field is
     (--  Timer Mode. Incremented every rising APB bus clock edge.
      Timer,
      --  Counter Mode rising edge. TC is incremented on rising edges on the CAP
--  input selected by bits 3:2.
      Counter_Rising_Edge,
      --  Counter Mode falling edge. TC is incremented on falling edges on the CAP
--  input selected by bits 3:2.
      Counter_Falling_Edge,
      --  Counter Mode dual edge. TC is incremented on both edges on the CAP input
...
--  clears the timer (if bit 4 is set).
      Channel_0_Rising,
      --  Channel 0 Falling Edge. Falling edge of the signal on capture channel 0
--  clears the timer (if bit 4 is set).
      Channel_0_Falling,
      --  Channel 1 Rising Edge. Rising edge of the signal on capture channel 1
--  clears the timer (if bit 4 is set).
      Channel_1_Rising,
      --  Channel 1 Falling Edge. Falling edge of the signal on capture channel 1
--  clears the timer (if bit 4 is set).
      Channel_1_Falling,
      --  Channel 2 Rising Edge. Rising edge of the signal on capture channel 2
--  clears the timer (if bit 4 is set).
      Channel_2_Rising,
      --  Channel 2 Falling Edge. Falling edge of the signal on capture channel 2
--  clears the timer (if bit 4 is set).
      Channel_2_Falling)
     with Size => 3;
   ...
   CTIMER0_Periph : aliased CTIMER_Peripheral
     with Import, Address => CTIMER0_Base;

   --  Standard counter/timers (CTIMER0 to 4)
   CTIMER1_Periph : aliased CTIMER_Peripheral
     with Import, Address => CTIMER1_Base;

   --  Standard counter/timers (CTIMER0 to 4)
   CTIMER2_Periph : aliased CTIMER_Peripheral
     with Import, Address => CTIMER2_Base;

   --  Standard counter/timers (CTIMER0 to 4)
   CTIMER3_Periph : aliased CTIMER_Peripheral
     with Import, Address => CTIMER3_Base;

   --  Standard counter/timers (CTIMER0 to 4)
   CTIMER4_Periph : aliased CTIMER_Peripheral
     with Import, Address => CTIMER4_Base;
