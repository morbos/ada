...
package NXP_SVD.HASHCRYPT is
   ...
   type CTRL_DMA_I_Field is
     (--  DMA is not used. Processor writes the necessary words when WAITING is set
--  (interrupts), unless AHB Master is used.
      Not_Used,
   ...
   type CTRL_DMA_O_Field is
     (--  DMA is not used. Processor reads the digest/output in response to DIGEST
--  interrupt.
      Notused)
     with Size => 1;
...
--  cryptographic uses, this is not set if IsLast is set nor will it set until
--  at least 1 word is read of the output.
      Not_Waiting,
...
--  needed.
      Not_Need,
      --  IV/Nonce is needed and INDATA/ALIAS will be accepted as IV/Nonce. Will also
--  set WAITING.
      Need)
     with Size => 1;
   ...
   type INTENSET_DIGEST_Field is
     (--  Will not interrupt when Digest is ready
      No_Interrupt,
      --  Will interrupt when Digest is ready. Interrupt cleared by writing more
--  data, starting a new Hash, or disabling (done).
      Interrupt)
     with Size => 1;
   ...
   type MEMCTRL_MASTER_Field is
     (--  Mastering is not used and the normal DMA or Interrupt based model is used
--  with INDATA.
      Not_Used,
...
--  non-secure requests.
      Unlock,
      --  Locks to the current security level. AHB Master will issue requests at this
--  level.
      Lock)
     with Size => 2;
   ...
   HASHCRYPT_Periph : aliased HASHCRYPT_Peripheral
     with Import, Address => HASHCRYPT_Base;
