...
package NXP_SVD.SCT is
   ...
   type CONFIG_CLKMODE_Field is
     (--  System Clock Mode. The system clock clocks the entire SCT module including
--  the counter(s) and counter prescalers.
      System_Clock_Mode,
      --  Sampled System Clock Mode. The system clock clocks the SCT module, but the
--  counter and prescalers are only enabled to count when the designated edge
--  is detected on the input selected by the CKSEL field. The minimum pulse
--  width on the selected clock-gate input is 1 bus clock period. This mode is
...
--  synchronized to the system clock. The minimum pulse width on the clock
--  input is 1 bus clock period. This mode is the low-power, sampled-clock
--  mode.
...
--  switched synchronously to the SCT input clock - not the system clock. The
--  input clock rate must be at least half the system clock rate and can be the
--  same or faster than the system clock.
      Asynchronous_Mode)
     with Size => 2;
   ...
   type CTRL_BIDIR_L_Field is
     (--  Up. The counter counts up to a limit condition, then is cleared to zero.
      Up,
      --  Up-down. The counter counts up to a limit, then counts down to a limit
--  condition or to 0.
      Up_Down)
     with Size => 1;
   ...
   type CTRL_BIDIR_H_Field is
     (--  The H counter counts up to its limit condition, then is cleared to zero.
      Up,
      --  The H counter counts up to its limit, then counts down to a limit condition
--  or to 0.
      Up_Down)
     with Size => 1;
...
--  counting down.
      L_Reversed,
      --  Set and clear are reversed when counter H is counting down. Do not use if
--  UNIFY = 1.
      H_Reversed)
     with Size => 2;
...
--  direction.
      ...
      --  Counting up. This event is triggered only during up-counting when BIDIR =
--  1.
      Counting_Up,
