...
--  starvation sources and puts the
   ...
   type PWD_TXPWDIBIAS_Field is
     (--  Normal operation.
      VALUE0,
      --  Power-down the USB PHY current bias block for the transmitter. This bit
--  should be set only when the
      VALUE1)
     with Size => 1;
   ...
   type PWD_RXPWDRX_Field is
     (--  Normal operation.
      VALUE0,
      --  Power-down the entire USB PHY receiver block except for the full-speed
...
--  full-speed differential receiver
   ...
   type USB1_VBUS_DETECT_VBUS_OVERRIDE_EN_Field is
     (--  Use the results of the internal VBUS_VALID and Session Valid comparators
--  for VBUS_VALID, AVALID, BVALID, and SESSEND (Default)
      VALUE0,
...
--  controller (Default)
      VALUE0,
      --  Use the Session Valid comparator results for signal reported to the USB
--  controller
      VALUE1,
      --  Use the Session Valid comparator results for signal reported to the USB
--  controller
   ...
   type USB1_VBUS_DETECT_VBUSVALID_TO_SESSVALID_Field is
     (--  Use the VBUS_VALID comparator for VBUS_VALID results
      VALUE0,
      --  Use the Session End comparator for VBUS_VALID results. The Session End
--  threshold is >0.8V and <4.0V.
      VALUE1)
     with Size => 1;
...
--  threshold is >0.8V and <4.0V.
   ...
   type ANACTRL_DEV_PULLDOWN_Field is
     (--  The 15kohm nominal pulldowns on the USB_DP and USB_DM pinsare disabled in
--  device mode.
      VALUE0,
      --  The 15kohm nominal pulldowns on the USB_DP and USB_DM pinsare enabled in
--  device mode.
      VALUE1)
     with Size => 1;
