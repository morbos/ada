...
package NXP_SVD.I2C is
   ...
   type CFG_TIMEOUTEN_Field is
     (--  Disabled. Time-out function is disabled.
      Disabled,
      --  Enabled. Time-out function is enabled. Both types of time-out flags will be
--  generated and will cause interrupts if they are enabled. Typically, only
--  one time-out will be used in a system.
      Enabled)
     with Size => 1;
   ...
   type CFG_MONCLKSTR_Field is
     (--  Disabled. The Monitor function will not perform clock stretching. Software
--  or DMA may not always be able to read data provided by the Monitor function
--  before it is overwritten. This mode may be used when non-invasive
--  monitoring is critical.
      Disabled,
      --  Enabled. The Monitor function will perform clock stretching in order to
--  ensure that software or DMA can read all incoming data supplied by the
--  Monitor function.
      Enabled)
     with Size => 1;
   ...
   type CFG_HSCAPABLE_Field is
     (--  Fast-mode plus. The I 2C interface will support Standard-mode, Fast-mode,
--  and Fast-mode Plus, to the extent that the pin electronics support these
--  modes. Any changes that need to be made to the pin controls, such as
--  changing the drive strength or filtering, must be made by software via the
--  IOCON register associated with each I2C pin,
      Fast_Mode_Plus,
      --  High-speed. In addition to Standard-mode, Fast-mode, and Fast-mode Plus,
--  the I 2C interface will support High-speed mode to the extent that the pin
--  electronics support these modes. See Section 25.7.2.2 for more information.
      High_Speed)
     with Size => 1;
   ...
   type STAT_MSTPENDING_Field is
     (--  In progress. Communication is in progress and the Master function is busy
--  and cannot currently accept a command.
      In_Progress,
...
--  transmit data or the NACK bit.
   ...
   type STAT_MSTSTATE_Field is
     (--  Idle. The Master function is available to be used for a new transaction.
      Idle,
      --  Receive ready. Received data available (Master Receiver mode). Address plus
--  Read was previously sent and Acknowledged by slave.
      Receive_Ready,
      --  Transmit ready. Data can be transmitted (Master Transmitter mode). Address
--  plus Write was previously sent and Acknowledged by slave.
   ...
   type STAT_MSTARBLOSS_Field is
     (--  No Arbitration Loss has occurred.
      No_Loss,
      --  Arbitration loss. The Master function has experienced an Arbitration Loss.
--  At this point, the Master function has already stopped driving the bus and
--  gone to an idle state. Software can respond by doing nothing, or by sending
--  a Start in order to attempt to gain control of the bus when it next becomes
--  idle.
      Arbitration_Loss)
     with Size => 1;
   ...
   type STAT_MSTSTSTPERR_Field is
     (--  No Start/Stop Error has occurred.
      No_Error,
      --  The Master function has experienced a Start/Stop Error. A Start or Stop was
--  detected at a time when it is not allowed by the I2C specification. The
--  Master interface has stopped driving the bus and gone to an idle state, no
--  action is required. A request for a Start could be made, or software could
--  attempt to insure that the bus has not stalled.
      Error)
     with Size => 1;
   ...
   type STAT_SLVPENDING_Field is
     (--  In progress. The Slave function does not currently need service.
      In_Progress,
      --  Pending. The Slave function needs service. Information on what is needed
--  can be found in the adjacent SLVSTATE field.
      Pending)
     with Size => 1;
   ...
   type STAT_SLVSTATE_Field is
     (--  Slave address. Address plus R/W received. At least one of the four slave
...
--  Deep-Sleep or Power-down mode cannot be entered at this time.
      Stretching,
      --  Not stretching. The slave function is not currently stretching the I 2C bus
--  clock. Deep-sleep or Power-down mode could be entered at this time.
   ...
   type STAT_SLVDESEL_Field is
     (--  Not deselected. The Slave function has not become deselected. This does not
--  mean that it is currently selected. That information can be found in the
--  SLVSEL flag.
      Not_Deselected,
      --  Deselected. The Slave function has become deselected. This is specifically
--  caused by the SLVSEL flag changing from 1 to 0. See the description of
--  SLVSEL for details on when that event occurs.
   ...
   type STAT_MONOV_Field is
     (--  No overrun. Monitor data has not overrun.
      No_Overrun,
      --  Overrun. A Monitor data overrun has occurred. This can only happen when
--  Monitor clock stretching not enabled via the MONCLKSTR bit in the CFG
--  register. Writing 1 to this bit clears the flag.
      Overrun)
     with Size => 1;
...
--  This must done after writing transmit data, reading received data, or any
--  other housekeeping related to the next bus operation.
      Continue)
     with Size => 1;
...
--  this I2C master is generating Acknowledge bits in Master Receiver mode, the
--  acknowledge is generated automatically.
   ...
   type SLVCTL_SLVCONTINUE_Field is
     (--  No effect.
      No_Effect,
      --  Continue. Informs the Slave function to continue to the next operation, by
--  clearing the SLVPENDING flag in the STAT register. This must be done after
--  writing transmit data, reading received data, or any other housekeeping
--  related to the next bus operation. Automatic Operation has different
...
--  reception.
   ...
   type SLVCTL_AUTOACK_Field is
     (--  Normal, non-automatic operation. If AUTONACK = 0, an SlvPending interrupt
--  is generated when a matching address is received. If AUTONACK = 1, received
--  addresses are NACKed (ignored).
      Normal,
      --  A header with matching SLVADR0 and matching direction as set by
--  AUTOMATCHREAD will be ACKed immediately, allowing the master to move on to
--  the data bytes. If the address matches SLVADR0, but the direction does not
--  match AUTOMATCHREAD, the behavior will depend on the AUTONACK bit in the
--  SLVADR0 register: if AUTONACK is set, then it will be Nacked; else if
...
--  AUTOACK is set, it matches SLVADRn, and AUTOMATCHREAD matches the
--  direction.
      Automatic)
     with Size => 1;
   ...
   type SLVQUAL0_QUALMODE0_Field is
     (--  Mask. The SLVQUAL0 field is used as a logical mask for matching address 0.
      Mask,
      --  Extend. The SLVQUAL0 field is used to extend address 0 matching in a range
--  of addresses.
      Extend)
     with Size => 1;
   ...
   type MONRXDAT_MONSTART_Field is
     (--  No start detected. The Monitor function has not detected a Start event on
--  the I2C bus.
      No_Start_Detected,
      --  Start detected. The Monitor function has detected a Start event on the I2C
--  bus.
      Start_Detected)
     with Size => 1;
...
--  acknowledged by at least one master or slave receiver.
      Acknowledged,
      --  Not acknowledged. The data currently being provided by the Monitor function
--  was not acknowledged by any receiver.
      Not_Acknowledged)
     with Size => 1;
