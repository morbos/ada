...
package NXP_SVD.I2S is
   ...
   type CFG1_MAINENABLE_Field is
     (--  All I 2S channel pairs in this Flexcomm are disabled and the internal state
--  machines, counters, and flags are reset. No other channel pairs can be
--  enabled.
      Disabled,
      --  This I 2S channel pair is enabled. Other channel pairs in this Flexcomm may
--  be enabled in their individual PAIRENABLE bits.
      Enabled)
     with Size => 1;
   ...
   type CFG1_DATAPAUSE_Field is
     (--  Normal operation, or resuming normal operation at the next frame if the I2S
...
--  DATAPAUSED in STAT = 1.
   ...
   type CFG1_MSTSLVCFG_Field is
     (--  Normal slave mode, the default mode. SCK and WS are received from a master
--  and used to transmit or receive data.
      Normal_Slave_Mode,
      --  WS synchronized master. WS is received from another master and used to
--  synchronize the generation of SCK, when divided from the Flexcomm function
--  clock.
      Ws_Sync_Master,
      --  Master using an existing SCK. SCK is received and used directly to generate
--  WS, as well as transmitting or receiving data.
      Master_Using_Sck,
      --  Normal master mode. SCK and WS are generated so they can be sent to one or
--  more slave devices.
      Normal_Master)
     with Size => 2;
...
--  enabled channel pair) one piece of left channel data occurring during the
--  first phase, and one pieces of right channel data occurring during the
--  second phase. In this mode, the data region begins one clock after the
--  leading WS edge for the frame. For a 50% WS duty cycle, FRAMELEN must
--  define an even number of I2S clocks for the frame. If FRAMELEN defines an
--  odd number of clocks per frame, the extra clock will occur on the right.
      Classic_Mode,
      --  DSP mode where WS has a 50% duty cycle. See remark for mode 0.
      Dsp_Mode_Ws_50_Dutycycle,
      --  DSP mode where WS has a one clock long pulse at the beginning of each data
--  frame.
      Dsp_Mode_Ws_1_Clock,
      --  DSP mode where WS has a one data slot long pulse at the beginning of each
--  data frame.
      Dsp_Mode_Ws_1_Data)
     with Size => 2;
...
--  channel.
      Right_High,
      --  The right channel is taken from the low part of the FIFO data. For example,
--  when data is 16 bits, FIFO bits 15:0 are used for the right channel.
      Right_Low)
     with Size => 1;
   ...
   type CFG1_LEFTJUST_Field is
     (--  Data is transferred between the FIFO and the I2S serializer/deserializer
--  right justified, i.e. starting from bit 0 and continuing to the position
--  defined by DATALEN. This would correspond to right justified data in the
--  stream on the data bus.
      Right_Justified,
      --  Data is transferred between the FIFO and the I2S serializer/deserializer
--  left justified, i.e. starting from the MSB of the FIFO entry and continuing
--  for the number of bits defined by DATALEN. This would correspond to left
--  justified data in the stream on the data bus.
      Left_Justified)
     with Size => 1;
   ...
   type CFG1_ONECHANNEL_Field is
     (--  I2S data for this channel pair is treated as left and right channels.
      Dual_Channel,
      --  I2S data for this channel pair is treated as a single channel, functionally
--  the left channel for this pair. In mode 0 only, the right side of the frame
--  begins at POSITION = 0x100. This is because mode 0 makes a clear
--  distinction between the left and right sides of the frame. When ONECHANNEL
--  = 1, the single channel of data may be placed on the right by setting
--  POSITION to 0x100 + the data position within the right side (e.g. 0x108
--  would place data starting at the 8th clock after the middle of the frame).
--  In other modes, data for the single channel of data is placed at the clock
--  defined by POSITION.
      Single_Channel)
     with Size => 1;
...
--  (standard for I2S).
   ...
   type CFG1_WS_POL_Field is
     (--  Data frames begin at a falling edge of WS (standard for classic I2S).
      Not_Inverted,
      --  WS is inverted, resulting in a data frame beginning at a rising edge of WS
--  (standard for most 'non-classic' variations of I2S).
      Inverted)
     with Size => 1;
   ...
   type STAT_SLVFRMERR_Field is
     (--  No error has been recorded.
      No_Error,
      --  An error has been recorded for some channel pair that is operating in slave
--  mode. ERROR is cleared by writing a 1 to this bit position.
      Error)
     with Size => 1;
   ...
   type STAT_DATAPAUSED_Field is
     (--  Data is not currently paused. A data pause may have been requested but is
--  not yet in force, waiting for an allowed pause point. Refer to the
--  description of the DATAPAUSE control bit in the CFG1 register.
      Not_Paused,
   ...
   type FIFOCFG_TXI2SE0_Field is
     (--  If the TX FIFO becomes empty, the last value is sent. This setting may be
--  used when the data length is 24 bits or less, or when MONO = 1 for this
--  channel pair.
      Last_Value,
      --  If the TX FIFO becomes empty, 0 is sent. Use if the data length is greater
--  than 24 bits or if zero fill is preferred.
      Zero)
     with Size => 1;
...
--  in the FIFOTRIG register.
   ...
   I2S2_Periph : aliased I2S_Peripheral
     with Import, Address => I2S2_Base;

   --  I2S interface
   I2S3_Periph : aliased I2S_Peripheral
     with Import, Address => I2S3_Base;

   --  I2S interface
   I2S4_Periph : aliased I2S_Peripheral
     with Import, Address => I2S4_Base;

   --  I2S interface
   I2S5_Periph : aliased I2S_Peripheral
     with Import, Address => I2S5_Base;

   --  I2S interface
   I2S6_Periph : aliased I2S_Peripheral
     with Import, Address => I2S6_Base;

   --  I2S interface
   I2S7_Periph : aliased I2S_Peripheral
     with Import, Address => I2S7_Base;
