...
package NXP_SVD.USART is
   ...
   type CFG_ENABLE_Field is
     (--  Disabled. The USART is disabled and the internal state machine and counters
--  are reset. While Enable = 0, all USART interrupts and DMA transfers are
--  disabled. When Enable is set again, CFG and most other control bits remain
...
--  because the transmitter has been reset and is therefore available.
   ...
   type CFG_DATALEN_Field is
     (--  7 bit Data length.
      ...
      --  8 bit Data length.
      Bit_8,
      --  9 bit data length. The 9th bit is commonly used for addressing in multidrop
--  mode. See the ADDRDET bit in the CTL register.
      Bit_9)
     with Size => 2;
   ...
   type CFG_PARITYSEL_Field is
     (--  No parity.
      No_Parity,
      --  Even parity. Adds a bit to each character such that the number of 1s in a
--  transmitted character is even, and the number of 1s in a received character
--  is expected to be even.
      Even_Parity,
      --  Odd parity. Adds a bit to each character such that the number of 1s in a
--  transmitted character is odd, and the number of 1s in a received character
...
--  communication.
   ...
   type CFG_MODE32K_Field is
     (--  Disabled. USART uses standard clocking.
      Disabled,
      --  Enabled. USART uses the 32 kHz clock from the RTC oscillator as the clock
--  source to the BRG, and uses a special bit clocking scheme.
      Enabled)
     with Size => 1;
...
--  control signal.
      Disabled,
      --  Flow control enabled. The transmitter uses the CTS input (or RTS output in
--  loopback mode) for flow control purposes.
      Enabled)
     with Size => 1;
   ...
   type CFG_LOOP_Field is
     (--  Normal operation.
      Normal,
      --  Loopback mode. This provides a mechanism to perform diagnostic loopback
--  testing for USART data. Serial data from the transmitter (Un_TXD) is
--  connected internally to serial input of the receive (Un_RXD). Un_TXD and
--  Un_RTS activity will also appear on external pins if these functions are
--  configured to appear on device pins. The receiver RTS signal is also looped
--  back to CTS and performs flow control if enabled by CTSEN.
      Loopback)
     with Size => 1;
   ...
   type CFG_OETA_Field is
     (--  Disabled. If selected by OESEL, the Output Enable signal deasserted at the
--  end of the last stop bit of a transmission.
      Disabled,
      --  Enabled. If selected by OESEL, the Output Enable signal remains asserted
--  for one character time after the end of the last stop bit of a
--  transmission. OE will also remain asserted if another transmit begins
--  before it is deasserted.
   ...
   type CFG_AUTOADDR_Field is
     (--  Disabled. When addressing is enabled by ADDRDET, address matching is done
--  by software. This provides the possibility of versatile addressing (e.g.
--  respond to more than one address).
      Disabled,
      --  Enabled. When addressing is enabled by ADDRDET, address matching is done by
--  hardware, using the value in the ADDR register as the address to match.
      Enabled)
     with Size => 1;
   ...
   type CFG_OESEL_Field is
     (--  Standard. The RTS signal is used as the standard flow control function.
      Standard,
      --  RS-485. The RTS signal configured to provide an output enable signal to
--  control an RS-485 transceiver.
      Rs_485)
     with Size => 1;
   ...
   type CFG_RXPOL_Field is
     (--  Standard. The RX signal is used as it arrives from the pin. This means that
--  the RX rest value is 1, start bit is 0, data is not inverted, and the stop
--  bit is 1.
      Standard,
      --  Inverted. The RX signal is inverted before being used by the USART. This
--  means that the RX rest value is 0, start bit is 1, data is inverted, and
--  the stop bit is 0.
      Inverted)
     with Size => 1;
   ...
   type CFG_TXPOL_Field is
     (--  Standard. The TX signal is sent out without change. This means that the TX
--  rest value is 1, start bit is 0, data is not inverted, and the stop bit is
--  1.
      Standard,
      --  Inverted. The TX signal is inverted by the USART before being sent out.
--  This means that the TX rest value is 0, start bit is 1, data is inverted,
--  and the stop bit is 0.
   ...
   type CTL_TXBRKEN_Field is
     (--  Normal operation.
      Normal,
      --  Continuous break. Continuous break is sent immediately when this bit is
--  set, and remains until this bit is cleared. A break may be sent without
--  danger of corrupting any currently transmitting character if the
--  transmitter is first disabled (TXDIS in CTL is set) and then waiting for
--  the transmitter to be disabled (TXDISINT in STAT = 1) before writing 1 to
--  TXBRKEN.
      Continous)
     with Size => 1;
   ...
   type CTL_ADDRDET_Field is
     (--  Disabled. The USART presents all incoming data.
      Disabled,
      --  Enabled. The USART receiver ignores incoming data that does not have the
--  most significant bit of the data (typically the 9th bit) = 1. When the data
--  MSB bit = 1, the receiver treats the incoming data normally, generating a
--  received data interrupt. Software can then check the data to see if this is
--  an address that should be handled. If it is, the ADDRDET bit is cleared by
...
--  transmitted is complete. This feature can be used to facilitate software
--  flow control.
      Disabled)
     with Size => 1;
   ...
   type CTL_CC_Field is
     (--  Clock on character. In synchronous mode, SCLK cycles only when characters
--  are being sent on Un_TXD or to complete a character that is being received.
      Clock_On_Character,
      --  Continuous clock. SCLK runs continuously in synchronous mode, allowing
--  characters to be received on Un_RxD independently from transmission on
--  Un_TXD).
      Continous_Clock)
     with Size => 1;
   ...
   type CTL_CLRCCONRX_Field is
     (--  No effect. No effect on the CC bit.
      No_Effect,
      --  Auto-clear. The CC bit is automatically cleared when a complete character
--  has been received. This bit is cleared at the same time.
      Auto_Clear)
     with Size => 1;
   ...
   type CTL_AUTOBAUD_Field is
     (--  Disabled. USART is in normal operating mode.
      Disabled,
      --  Enabled. USART is in autobaud mode. This bit should only be set when the
--  USART receiver is idle. The first start bit of RX is measured and used the
--  update the BRG register to match the received data rate. AUTOBAUD is
--  cleared once this process is complete, or if there is an AERR.
      Enabled)
     with Size => 1;
   ...
   type FIFOCFG_DMATX_Field is
     (--  DMA is not used for the transmit function.
      Disabled,
      --  Trigger DMA for the transmit function if the FIFO is not full. Generally,
--  data interrupts would be disabled if DMA is enabled.
      Enabled)
     with Size => 1;
   ...
   type FIFOCFG_DMARX_Field is
     (--  DMA is not used for the receive function.
      Disabled,
      --  Trigger DMA for the receive function if the FIFO is not empty. Generally,
--  data interrupts would be disabled if DMA is enabled.
      Enabled)
     with Size => 1;
   ...
   type FIFOCFG_WAKETX_Field is
     (--  Only enabled interrupts will wake up the device form reduced power modes.
      Disabled,
      --  A device wake-up for DMA will occur if the transmit FIFO level reaches the
--  value specified by TXLVL in FIFOTRIG, even when the TXLVL interrupt is not
--  enabled.
      Enabled)
     with Size => 1;
   ...
   type FIFOCFG_WAKERX_Field is
     (--  Only enabled interrupts will wake up the device form reduced power modes.
      Disabled,
      --  A device wake-up for DMA will occur if the receive FIFO level reaches the
--  value specified by RXLVL in FIFOTRIG, even when the RXLVL interrupt is not
--  enabled.
      Enabled)
     with Size => 1;
   ...
   type FIFOTRIG_TXLVLENA_Field is
     (--  Transmit FIFO level does not generate a FIFO level trigger.
      Disabled,
      --  An trigger will be generated if the transmit FIFO level reaches the value
--  specified by the TXLVL field in this register.
      Enabled)
     with Size => 1;
...
--  specified by the RXLVL field in this register.
      Enabled)
     with Size => 1;
   ...
   type FIFOINTENSET_TXLVL_Field is
     (--  No interrupt will be generated based on the TX FIFO level.
      Disabled,
      --  If TXLVLENA in the FIFOTRIG register = 1, an interrupt will be generated
--  when the TX FIFO level decreases to the level specified by TXLVL in the
--  FIFOTRIG register.
      Enabled)
     with Size => 1;
   ...
   type FIFOINTENSET_RXLVL_Field is
     (--  No interrupt will be generated based on the RX FIFO level.
      Disabled,
      --  If RXLVLENA in the FIFOTRIG register = 1, an interrupt will be generated
--  when the when the RX FIFO level increases to the level specified by RXLVL
