...
--  interrupt function.
   ...
   type PMSRC_SRC0_Field is
     (--  Input 0. Selects the pin selected in the PINTSEL0 register as the source to
--  bit slice 0.
      Input0,
      --  Input 1. Selects the pin selected in the PINTSEL1 register as the source to
--  bit slice 0.
      Input1,
      --  Input 2. Selects the pin selected in the PINTSEL2 register as the source to
--  bit slice 0.
      Input2,
      --  Input 3. Selects the pin selected in the PINTSEL3 register as the source to
--  bit slice 0.
      Input3,
      --  Input 4. Selects the pin selected in the PINTSEL4 register as the source to
--  bit slice 0.
      Input4,
      --  Input 5. Selects the pin selected in the PINTSEL5 register as the source to
--  bit slice 0.
      Input5,
      --  Input 6. Selects the pin selected in the PINTSEL6 register as the source to
--  bit slice 0.
      Input6,
      --  Input 7. Selects the pin selected in the PINTSEL7 register as the source to
--  bit slice 0.
      Input7)
     with Size => 3;
...
--  interrupt 6 in the NVIC is raised if the minterm evaluates as true.
   ...
   type PMCFG_CFG0_Field is
     (--  Constant HIGH. This bit slice always contributes to a product term match.
      Constant_High,
      --  Sticky rising edge. Match occurs if a rising edge on the specified input
--  has occurred since the last time the edge detection for this bit slice was
--  cleared. This bit is only cleared when the PMCFG or the PMSRC registers are
--  written to.
      Sticky_Rising_Edge,
      --  Sticky falling edge. Match occurs if a falling edge on the specified input
--  has occurred since the last time the edge detection for this bit slice was
--  cleared. This bit is only cleared when the PMCFG or the PMSRC registers are
--  written to.
      Sticky_Falling_Edge,
      --  Sticky rising or falling edge. Match occurs if either a rising or falling
--  edge on the specified input has occurred since the last time the edge
--  detection for this bit slice was cleared. This bit is only cleared when the
--  PMCFG or the PMSRC registers are written to.
      Sticky_Rising_Falling_Edge,
      --  High level. Match (for this bit slice) occurs when there is a high level on
--  the input specified for this bit slice in the PMSRC register.
      High_Level,
      --  Low level. Match occurs when there is a low level on the specified input.
      Low_Level,
      --  Constant 0. This bit slice never contributes to a match (should be used to
--  disable any unused bit slices).
      Constant_Zero,
      --  Event. Non-sticky rising or falling edge. Match occurs on an event - i.e.
--  when either a rising or falling edge is first detected on the specified
--  input (this is a non-sticky version of value 0x3) . This bit is cleared
--  after one clock cycle.
   ...
   PINT_Periph : aliased PINT_Peripheral
     with Import, Address => PINT_Base;

   --  Pin interrupt and pattern match (PINT)
   SECPINT_Periph : aliased PINT_Peripheral
     with Import, Address => SECPINT_Base;
