...
package NXP_SVD.SPI is
   ...
   type CFG_ENABLE_Field is
     (--  Disabled. The SPI is disabled and the internal state machine and counters
--  are reset.
      Disabled,
...
--  signals are inputs, MISO is an output.
      Slave_Mode,
      --  Master mode. The SPI will operate in master mode. SCK, MOSI, and the SSEL
--  signals are outputs, MISO is an input.
      Master_Mode)
     with Size => 1;
   ...
   type CFG_CPHA_Field is
     (--  Change. The SPI captures serial data on the first clock transition of the
--  transfer (when the clock changes away from the rest state). Data is changed
--  on the following edge.
      Change,
      --  Capture. The SPI changes serial data on the first clock transition of the
--  transfer (when the clock changes away from the rest state). Data is
--  captured on the following edge.
   ...
   type INTENSET_SSAEN_Field is
     (--  Disabled. No interrupt will be generated when any Slave Select transitions
--  from deasserted to asserted.
      Disabled,
      --  Enabled. An interrupt will be generated when any Slave Select transitions
--  from deasserted to asserted.
      Enabled)
     with Size => 1;
...
--  in the FIFOTRIG register.
   ...
   type FIFOWR_EOT_Field is
     (--  SSEL not deasserted. This piece of data is not treated as the end of a
--  transfer. SSEL will not be deasserted at the end of this data.
      Not_Deasserted,
      --  SSEL deasserted. This piece of data is treated as the end of a transfer.
--  SSEL will be deasserted at the end of this piece of data.
      Deasserted)
     with Size => 1;
   ...
   type FIFOWR_EOF_Field is
     (--  Data not EOF. This piece of data transmitted is not treated as the end of a
--  frame.
      Not_Eof,
