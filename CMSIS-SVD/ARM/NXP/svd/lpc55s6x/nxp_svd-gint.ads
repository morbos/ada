...
package NXP_SVD.GINT is
   ...
   type CTRL_COMB_Field is
     (--  Or. OR functionality: A grouped interrupt is generated when any one of the
--  enabled inputs is active (based on its programmed polarity).
      Or_k,
      --  And. AND functionality: An interrupt is generated when all enabled bits are
--  active (based on their programmed polarity).
      And_k)
     with Size => 1;
   ...
   GINT0_Periph : aliased GINT_Peripheral
     with Import, Address => GINT0_Base;

   --  Group GPIO input interrupt (GINT0/1)
   GINT1_Periph : aliased GINT_Peripheral
     with Import, Address => GINT1_Base;
